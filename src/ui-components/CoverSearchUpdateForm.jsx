...
export default function CoverSearchUpdateForm(props) {
  const {
    id: idProp,
    coverSearch,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = coverSearchRecord
      ? { ...initialValues, ...coverSearchRecord }
      : initialValues;
    setFirstName(cleanValues.FirstName);
    setLastName(cleanValues.LastName);
    setExperience(cleanValues.Experience);
    setPrevPos(cleanValues.PrevPos);
    setWhy(cleanValues.Why);
    setSkills(cleanValues.Skills);
    setCompNpos(cleanValues.CompNpos);
    setErrors({});
  };
  const [coverSearchRecord, setCoverSearchRecord] = React.useState(coverSearch);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CoverSearch, idProp)
        : coverSearch;
      setCoverSearchRecord(record);
    };
    queryData();
  }, [idProp, coverSearch]);
  React.useEffect(resetStateValues, [coverSearchRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CoverSearch.copyOf(coverSearchRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CoverSearchUpdateForm")}
      {...rest}
    >
      <TextField
        label="First name"
